...
class DriverLoaderStrategy(GameObject):
    ...
    @staticmethod
    def load(cars, r_p, car_name, track, race, player_car_name,
             player_car_names, s_p, aipoller, callback, players):
        # unused car_name
        if not cars: return callback()
        ...
        car_cls = AiCar if no_p and race.__class__.__name__ == \
            'RaceSinglePlayer' else car_cls
        race.logic.cars += [DriverLoaderStrategy.actual_load(
            cars, car, r_p, track, race, car_cls, player_car_name,
            player_car_names, s_p, aipoller, callback, 0, players)]
        return None

    @staticmethod
    def actual_load(
            cars, load_car_name, r_p, track, race, car_cls, player_car_name,
            player_car_names, seas_p, aipoller, callback, player_car_idx,
            players):
        ...
        player_car_name = [player.driver for player in players
                           if player.kind == Player.human][0]
        if load_car_name == player_car_name:
            if r_p.start_wp:
                wp = [wayp for wayp in track.phys.waypoints
                      if wayp.get_name() == 'Waypoint%s' % r_p.start_wp][0]
                pos = wp.node.get_pos()
        car_props = CarProps(
            r_p, load_car_name, pos, hpr,
            lambda: DriverPlayerLoaderStrategy.load(
                cars, r_p, load_car_name, track, race, player_car_name,
                player_car_names, seas_p, aipoller, callback, players),
            race, player.tuning.engine, player.tuning.tires,
            player.tuning.suspensions, race.track.phys.waypoints,
            race.track.phys.skidmark_col, aipoller)
        if player_car_idx == -1:
            car = car_cls(car_props, player_car_idx, player.tuning, players)
        else:
            car = car_cls(car_props, player_car_idx, player.tuning, players)
        return car
...
class DriverPlayerLoaderStrategy(GameObject):
    ...
    @staticmethod
    def load(loadcars, r_p, car_name, track, race, player_car_name,
             player_car_names, s_p, aipoller, callback, players):
        # unused car_name
        if not loadcars: return callback()
        eng = DriverLoaderStrategy.eng
        car = loadcars.pop(0)
        local_mp = s_p.kind == 'localmp'
        if local_mp and car in player_car_names or not local_mp and \
                car == player_car_name or s_p.kind == 'onlinemp' and \
                car in player_car_names:
            if r_p.a_i == 1 or r_p.a_i == 2 and car != player_car_names[0]:
                car_cls = AiCarPlayer
            else:
                car_cls = CarPlayerLocalMP if car in player_car_names and \
                    player_car_names != [player_car_name] else CarPlayer
            ...
            idx = 0 if car_cls in [CarPlayerServer, CarPlayerClient] else \
                player_car_names.index(car)
            race.logic.player_cars += [DriverLoaderStrategy.actual_load(
                loadcars, car, r_p, track, race, car_cls, player_car_name,
                player_car_names, s_p, aipoller, callback, idx, players)]
        else:
            car_cls = Car
            if eng.server.is_active or eng.client.is_active:
                car_cls = NetworkCar  # if car in player_cars else Car
            no_p = car not in player_car_names
            car_cls = AiCar if no_p and \
                race.__class__.__name__ == 'RaceSinglePlayer' else car_cls
            race.logic.cars += [DriverLoaderStrategy.actual_load(
                loadcars, car, r_p, track, race, car_cls, player_car_name,
                player_car_names, s_p, aipoller, callback, -1, players)]
        return None
