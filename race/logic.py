...
from yracing.track.track import Track
from yracing.car.ai import CarAiPoller
from yracing.car.car import AiCar
from yracing.driver.logic import DriverPlayerLoaderStrategy
...
class RaceLogic(LogicColleague):
    ...
    def set_display_regions(self):
        if len(base.win.get_active_display_regions()) > 4:
            list(map(base.win.remove_display_region,
                     base.win.get_active_display_regions()[1:-2]))
        ...
        self.mediator.event.accept('aspectRatioChanged',
    ...
    def on_aspect_ratio_changed(self):
        if len(self.player_cars) == 1:
            ...
            list(map(lambda cam: cam.node().get_lens().set_aspect_ratio(a_r),
                     self.cameras))
        if len(self.player_cars) == 2:
    ...
    def start_play(self):
        ...
        # for player_car in self.player_cars:
        #     player_car.logic.camera.render_all(self.track.gfx.model)
        # # workaround for prepare_scene (panda3d 1.9)
        self.track.play_music()
        list(map(lambda car: car.reset_car(), self.all_cars))
        list(map(lambda car: car.start(), self.all_cars))
        list(map(lambda car: car.event.attach(self.on_rotate_all),
                 self.all_cars))
        self.mediator.gui.start()

    def is_ai_car(self, player_car):
        return self.props.a_i == 1 or \
            self.props.a_i == 2 and \
            player_car.name != self.props.season_props.player_car_names[0]
    ...
    def min_dist(self, car):
        ...
        for _car in [_car for _car in self.all_cars if _car != car]:
            dist = (car.gfx.nodepath.node.get_pos() -
                    _car.gfx.nodepath.node.get_pos()).length()
            distances += [dist]
    ...
    def on_frame(self):
        ...
        if self.props.a_i and self.props.ai_debug:
            self.track.gfx.set_curr_wp(
                self.player_cars[0].ai.curr_logic.curr_tgt_wp)  # for debug
        if self.mediator.fsm.getCurrentOrNextState() == 'Play':
            for player_car in self.player_cars:
                player_car.upd_ranking(self.ranking())
                if self.props.a_i and self.props.ai_debug:
                    player_car.gui.ai_panel.curr_wp = \
                        player_car.ai.curr_logic.curr_tgt_wp.get_name()[8:]
                    player_car.gui.ai_panel.curr_logic = \
                        player_car.ai.curr_logic.__class__.__name__
                    player_car.gui.ai_panel.curr_car_dot_traj = \
                        round(player_car.ai.curr_logic.car_dot_traj, 3)
                    player_car.gui.ai_panel.curr_obsts = \
                        player_car.ai.front_logic.get_obstacles()
                    player_car.gui.ai_panel.curr_obsts_back = \
                        player_car.ai.rear_logic.get_obstacles()
                    player_car.gui.ai_panel.curr_input = \
                        player_car.ai.get_input()
    ...
    def ranking(self):
        ...
        for car in cars:
            ...
            _info += [(car.name, car.laps_num, wp_num, dist)]
        sortfunc = lambda val: (val[1], val[2], val[3])
        ranking_info = list(reversed(sorted(_info, key=sortfunc)))
    ...
    def exit_play(self):
        ...
        self.track.destroy()
        list(map(lambda car: car.event.detach(self.on_rotate_all),
                 self.all_cars))
...
class RaceLogicClient(RaceLogic):
    ...
    def on_start_countdown(self):
        ...
        self.aux_launch_tsk = self.eng.do_later(
            .5, self.mediator.fsm.client_start_countdown)
